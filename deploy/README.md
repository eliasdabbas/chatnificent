...
TLS is provisioned by Let's Encrypt via `certbot --nginx`; the cert covers
`chatnificent.com` and `www.chatnificent.com`. Auto-renewal is handled by
the `certbot.timer` systemd unit (runs twice daily, renews when <30 days
remain). The 443 server block and HTTP→HTTPS redirect in `nginx.conf` were
generated by certbot and are marked with `# managed by Certbot` comments.
